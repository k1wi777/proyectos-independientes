...
const ModalDialog = forwardRef(function ModalDialog(
  { timeRemaining, timeTarget, onResetTimeRemaining },
  ref,
) {
  const dialog = useRef();
  const userHasLost = timeRemaining <= 0;
  const timeRemainingFormated = (timeRemaining / 1000).toFixed(2);
  const userScored = Math.round(
    (1 - timeRemaining / (timeTarget * 1000)) * 100,
  );
  ...
  const titleModal = userHasLost ? (
    ...
    <>
      puntuacion total <strong>{userScored} / 100</strong>{" "}
    </>
  );
  const timerIsStoppedText = userHasLost ? (
    <>
      no has conseguido para el tiempo antes del fin de la cuenta atras. ¡Vuelve
      a probar suerte!
    </>
  ) : (
    <>
      {" "}
      Has parado el tiempo a falta de <strong>
  ...
  return createPortal (
    <>
      ...
      >
        ...
        <p>{timerIsStoppedText}</p>
        <form action="dialog" onSubmit={onResetTimeRemaining}>
          <button
    ...
    document.getElementById('modal') //a donde la voy a transportar
  );
});
